import type { BusinessType, CartItem, CartProduct, CartSummary, Product, StockStatus } from '../types';

/**
 * Ported line-for-line from frontend/src/cart/cartState.ts so the mobile cart behaves
 * identically to the web cart (same stock clamping, same dedupe key, same storage shape).
 */

export const CART_STORAGE_KEY = 'vault-cart-v1';
...
function parseItem(value: unknown): CartItem | null {
  ...
  if (
    !businesses.has(product.business as BusinessType) ||
    !statuses.has(product.status as StockStatus) ||
    !hasNumber(product.price) ||
